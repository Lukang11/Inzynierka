...
function NearbyEventsComponent({ userLocation,filter }) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const filteredEvents = filterEvents(allEvents, filter, userLocation, userHobbiesData);
  ...
  const filterEvents = (events, filter, userLocation, userHobbiesData) => {
    return events.filter((event) => {
      ...
      const isHobbyMatch = Array.isArray(event.relatedHobbiesName) &&
                           event.relatedHobbiesName.some(hobby => userHobbiesData.includes(hobby));
  
      // Wykluczenie wydarzeń bez danych geolokalizacyjnych, jeśli filtr jest aktywny
      if (filter && (!event.geoLocation || !event.geoLocation.latitude || !event.geoLocation.longitude)) {
        return false;
      }
