...
function EventBattler() {
  ...
  const updateParticipants = (obj) => { // funkcja aktualizacja po dołączeniu do rooma
    console.log("podaje to" , obj);
    setParticipants([[...participants,obj]]);
  }

  const messageListener = (obj) => { // funkcja zapisująca nową wiadomość
...
setMessages([...messages, obj]) // funkcja do zapisania wiadomosci
  ...
  useEffect(() => {  // nawiązujemy połączenie
    const newSocket = io("http://localhost:8002", {
        query: {
            "sender_id": user._id,
            "room_id" : "tu dodac id roomu ze sciezki" // uzupełnić
        },
    })
    setSocket(newSocket);
    return () => {
...
useEffect(() => { // odbieramy aktualizacje po dołączeniu kogos do rooma
  socket?.on("updateParticipants",updateParticipants)
},[updateParticipants])


useEffect(() => { // odbieranie wiadomości 
